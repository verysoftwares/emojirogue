...
function enemy_pathfind(pos,tgt)
    ...
    for i,v in ipairs(out) do
        ...
        if px-1>=cam.x and is_passable(posstr(px-1,py)) and not findany(out,posstr(px-1,py)) then ins(out,{posstr(px-1,py)}); out[#out].prev=v end
        if px-1>=cam.x and py-1>=cam.y and is_passable(posstr(px-1,py-1)) and not findany(out,posstr(px-1,py-1)) then ins(out,{posstr(px-1,py-1)}); out[#out].prev=v end
        if py-1>=cam.y and is_passable(posstr(px,py-1)) and not findany(out,posstr(px,py-1)) then ins(out,{posstr(px,py-1)}); out[#out].prev=v end
        if px+1<cam.x+24 and py-1>=cam.y and is_passable(posstr(px+1,py-1)) and not findany(out,posstr(px+1,py-1)) then ins(out,{posstr(px+1,py-1)}); out[#out].prev=v end
        if px+1<cam.x+24 and is_passable(posstr(px+1,py)) and not findany(out,posstr(px+1,py)) then ins(out,{posstr(px+1,py)}); out[#out].prev=v end
        if px+1<cam.x+24 and py+1<cam.y+12 and is_passable(posstr(px+1,py+1)) and not findany(out,posstr(px+1,py+1)) then ins(out,{posstr(px+1,py+1)}); out[#out].prev=v end
        if py+1<cam.y+12 and is_passable(posstr(px,py+1)) and not findany(out,posstr(px,py+1)) then ins(out,{posstr(px,py+1)}); out[#out].prev=v end
        if px-1>=cam.x and py+1<cam.y+12 and is_passable(posstr(px-1,py+1)) and not findany(out,posstr(px-1,py+1)) then ins(out,{posstr(px-1,py+1)}); out[#out].prev=v end
    end
